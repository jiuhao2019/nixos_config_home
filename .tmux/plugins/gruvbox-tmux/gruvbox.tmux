...
tmux set -g status-style bg="${THEME[bblack]}"
...
if [[ "$session_bg_enabled" == "on" ]]; then
    ...
    # tmux set -g status-left "#[fg=${THEME[bblack]},bg=${THEME[blue]},bold] #{?client_prefix,◉ ,#[dim]○}#[bold,nodim]#S "
    tmux set -g status-left "#[bg=${THEME[bblack]}] #{?client_prefix,#[fg=${THEME[byellow]}]◉,#[fg=${THEME[bblack]}]○} #[fg=${THEME[bblack]},bold]#S "
else
...
#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=${THEME[bblack]}] #{?#{==:#{pane_current_command},ssh},󰣀,} #[fg=${THEME[green]},bold,nodim]$window_number #W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane} #{?window_last_flag,,} "
# Unfocused
...
tmux set -g window-status-format "$RESET#[fg=${THEME[green]},bg=${THEME[bblack]}] #{?#{==:#{pane_current_command},ssh},󰣀,} #[fg=${THEME[foreground]},bold,nodim]$window_number #W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane} #{?window_last_flag,,} "
