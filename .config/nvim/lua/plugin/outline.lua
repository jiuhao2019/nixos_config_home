...
require("outline").setup({
	...
	symbols = {
		icon_fetcher = function(kind, bufnr, symbol)
			...
			local is_cpp = ft == "cpp" or ft == "cxx" or ft == "cc" or ft == "objc" or ft == "objcpp"

			if not (is_c or is_cpp) then
				return false
			end

			local detail = symbol and symbol.detail or ""

			-- Function
			if kind == "Function" then
				if detail:match("^static%s") then
					return "󰊕"
				end

				if detail:match("^extern%s") then
					return "󰏫"
				end
