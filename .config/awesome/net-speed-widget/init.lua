...
local WIDGET_DIR = HOME_DIR .. "/.config/awesome/net-speed-widget/"
...
local function convert_to_h(bytes)
	...
	elseif bits < 1000000000000 then
		...
		speed = bits / 1000000000000
		dim = "Tb/s"
	end

	return string.format("%.1f %s", speed/10, dim)
end
...
local function worker(user_args)
	...
	-- 改成你的实际网卡名称
	local interface = args.interface or "ens33"
	...
	local update_widget = function(widget, stdout)
		...
		for i, v in ipairs(cur_vals) do
			if i % 2 == 1 then
				cur_rx = cur_rx + tonumber(v)
			end

			if i % 2 == 0 then
				cur_tx = cur_tx + tonumber(v)
			end
		end
