...
(add-hook 'emacs-lisp-mode-hook
          (lambda ()
            (setq show-trailing-whitespace t)))
;; 记住文件离开时光标位置
(setq save-place-file "~/Downloads/emacs-temp-data/emacs_save-place")
(make-directory (file-name-directory save-place-file) t)
(save-place-mode 1)

;; 指定自动保存文件的文件名，也可以不用指定
(setq prescient-save-file (expand-file-name "~/Downloads/emacs-temp-data/prescient-save.el"))
...
(defun my-fix-prescient-lexical-binding ()
  (let ((file (expand-file-name
               "~/Downloads/emacs-temp-data/prescient-save.el")))
